...
class TestDb(DigitalMillietTestCase):
    ...
    def test_validate_annotation(self):
        file = os.path.join(
            ...
            'annotationtestfixtures.yml')
        with open(file, 'r') as (stream):
            data = yaml.load(stream)
        self.assertTrue(
            self.dm.commentaries.validate_annotation(
                data['valid']),
    ...
    def test_create_tag_annotation(self):
        ...
        with self.app.app_context():
            ...
            anno = self.dm.commentaries.create_tag_annotation(
                tag, target, person, date)
